    ...
    for predicate in body:
        if type(predicate) is Predicate.AtomicPredicate:
           if predicate.getTitle() in dataframes:
                df = predicate.filter(dataframes[predicate.getTitle()], head)
...
def create_fact(head, row):
    ...
    for key in row:
        ...
        if type(l) is str:
            ...
            data.append(row[key])
    return f"{head}({', '.join(data)})"

def print_evaluator(rules):
    print("\n----- Evaluation program ------\n")
    for rule in rules:
        for row in rules[rule]:
            print(f"{create_fact(rule, row)}")
        print("\n")
